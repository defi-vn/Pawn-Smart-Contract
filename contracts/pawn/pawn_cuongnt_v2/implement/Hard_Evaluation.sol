    ...
    ReentrancyGuardUpgradeable
{
        ...
        nonReentrant
    {
        ...
        // Check balance
        require(IERC20Upgradeable(mintingFeeAddress).balanceOf(msg.sender) >= (mintingNFTFee), "21"); // Your balance is not enough.
        
        require(IERC20Upgradeable(mintingFeeAddress).allowance(msg.sender, address(this)) >= (mintingNFTFee), "22"); // You have not approve DFY.

        // todo create nft

        // Tranfer minting fee to admin
        IERC20Upgradeable(mintingFeeAddress).transferFrom(msg.sender,addressAdmin , mintingNFTFee);
        ...
        // emit NFTEvent( ,_asset.owner, _nftCID);
    }
}